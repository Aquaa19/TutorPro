...
import { Search, Plus, Filter, Users, Calendar, IndianRupee, Phone, ArrowUpRight, GraduationCap, Clock, Trash2 } from 'lucide-react';
...
export default function StudentsScreen({
  ...
  const handleCreateStudent = (e: React.FormEvent) => {
    ...
    onAddStudent({
      ...
      subject: finalSubject || "General Core",
  ...
  const handleCreateBatch = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newBatchName || !newBatchTiming || newBatchDays.length === 0) {
      alert("Please fill in Batch Name, Timing and select at least one Scheduled Day.");
      return;
    }
    ...
    onAddBatch({
      ...
      days: newBatchDays,
      subject: newBatchSubject || "General Subjects"
    });
    ...
    <div className="space-y-6">
      ...
      {isAddingBatch && (
        <div className="bg-dark-card border border-white/5 rounded-3xl p-6 shadow-xl animate-slide-down">
          ...
          <form onSubmit={handleCreateBatch} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs text-slate-400 font-mono mb-1.5">Weekly Timing (e.g. 05:00 PM - 06:30 PM)</label>
              <input
                type="text"
            ...
            <div className="md:col-span-3 flex justify-end gap-2 pt-4 border-t border-white/5">
              <button
                type="submit"
      ...
      {isAddingStudent && (
        <div className="bg-dark-card border border-white/5 rounded-3xl p-6 shadow-xl animate-slide-down">
          ...
          <form onSubmit={handleCreateStudent} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div>
            ...
            <div>
              <label className="block text-xs text-slate-400 font-mono mb-1.5">Grade/Class Level (e.g. Grade 10)</label>
              <input
                type="text"
            ...
            <div className="sm:col-span-2 md:col-span-3 flex justify-end gap-2 pt-4 border-t border-white/5">
              <button
                type="submit"
            ...
            return (
              <div key={batch.id} className="bg-dark-card border border-white/5 rounded-3xl overflow-hidden shadow-xl">
                <div className="bg-white/[0.02] px-6 py-5 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-1 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10 font-mono text-[10px] text-slate-400 self-start md:self-auto uppercase tracking-wider">
